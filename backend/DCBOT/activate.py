...
async def genre(ctx, url: str = None):
    if not url or not ("https://www.youtube.com/" in url or "https://youtu.be/" in url):
    ...
    winner, lines, title = await loop.run_in_executor(None, blocking_task)
    ...
    # attach audio if file exists and is under 25 MB
    audio_files = []
    if os.path.exists(audio_path) and os.path.getsize(audio_path) < 25 * 1024 * 1024:
        audio_files = [discord.File(audio_path, filename="audio.mp3")]
