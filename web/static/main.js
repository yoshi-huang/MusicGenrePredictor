...
let voteSeq   = [];   // array of genre-index per second
...
// resolve the nearest non-transition genre before/after index i
...
// Oklab interpolation for perceptually smooth colour gradients
...
function drawTimeline(playPct) {
  ...
  for (const seg of segs) {
    ...
    if (seg.idx === -1) {
      // transition: Oklab gradient between prev and next genre
      ...
      // sample Oklab at several stops for a perceptually smooth gradient
      const stops = 8;
      ...
      if (segW > 36) {
        ...
        const pad = 4, th = 14, tw = Math.min(segW - 4, ctx.measureText(label).width + pad * 2);
...
function buildLabels(seq) {
  ...
  // t=0 plus the centre of every transition (-1) block
  const boundaries = [0];
  for (let i = 1; i < total; i++) {
    if (seq[i] === -1 && seq[i - 1] !== -1) {
      // find end of this -1 block
      let j = i;
      while (j < total && seq[j] === -1) j++;
      boundaries.push((i + j) / 2);  // centre of transition block
      i = j - 1;  // skip rest of block
    }
  }
...
// clicking the canvas also seeks
...
async function analyze() {
  ...
  try {
    ...
    // wait one frame so the card is visible before measuring width
